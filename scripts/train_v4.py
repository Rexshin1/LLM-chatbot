...
def main():
    ...
    if not os.path.exists(vocab_path):
        raise FileNotFoundError(f"Vocab file not found at {vocab_path}. Run build_data_v4 first.")
    ...
    # Override save_checkpoint to save with v4 suffix
    original_save = trainer.save_checkpoint
    def save_checkpoint_v4(epoch: int, train_loss: float, val_loss: float, filename: str):
        if filename == "best_model.pt":
            filename = "v4_best_model.pt"
        elif filename == "latest_model.pt":
            filename = "v4_latest_model.pt"
        original_save(epoch, train_loss, val_loss, filename=filename)
        
    trainer.save_checkpoint = save_checkpoint_v4
    
    # Resume v4 checkpoint if available
    latest_checkpoint_path = os.path.join(checkpoint_dir, "v4_latest_model.pt")
    ...
    print("Starting training on v4 dataset...")
