...
def main():
    ...
    if not os.path.exists(vocab_path):
        raise FileNotFoundError(f"Vocab file not found at {vocab_path}. Run build_data_v5 first.")
    ...
    # Override save_checkpoint to save with rexa_v5_instruction suffix
    original_save = trainer.save_checkpoint
    def save_checkpoint_v5(epoch: int, train_loss: float, val_loss: float, filename: str):
        if filename == "best_model.pt":
            filename = "rexa_v5_instruction.pt"
        elif filename == "latest_model.pt":
            filename = "rexa_v5_latest.pt"
        original_save(epoch, train_loss, val_loss, filename=filename)
        
    trainer.save_checkpoint = save_checkpoint_v5
    
    # Resume v5 checkpoint if available
    latest_checkpoint_path = os.path.join(checkpoint_dir, "rexa_v5_latest.pt")
    ...
    print("Starting training on v5 dataset...")
