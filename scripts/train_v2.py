...
def main():
    ...
    learning_rate = 3e-4
    weight_decay = 0.01
    epochs = 15  # Train slightly longer to learn the larger dataset
    grad_clip = 1.0
    ...
    if not os.path.exists(vocab_path):
        raise FileNotFoundError(f"Vocab file not found at {vocab_path}. Run build_data_v2 first.")
    ...
    # Trainer (saves checkpoints as v2 files)
    trainer = Trainer(
        model=model,
        optimizer=optimizer,
    ...
    # Helper to save checkpoints with _v2.pt suffix
    original_save_checkpoint = trainer.save_checkpoint
    def save_checkpoint_v2(epoch: int, train_loss: float, val_loss: float, filename: str):
        if filename == "best_model.pt":
            filename = "best_model_v2.pt"
        elif filename == "latest_model.pt":
            filename = "latest_model_v2.pt"
        original_save_checkpoint(epoch, train_loss, val_loss, filename=filename)
        
    trainer.save_checkpoint = save_checkpoint_v2
    
    # Load latest v2 checkpoint if it exists to resume
    latest_checkpoint_path = os.path.join(checkpoint_dir, "latest_model_v2.pt")
    ...
    print("Starting training on v2 dataset...")
